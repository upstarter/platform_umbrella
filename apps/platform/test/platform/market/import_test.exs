defmodule Platform.Market.ImportTest do
  ...
  describe "all/1" do
              ...
              %{
                ...
                transactions: [
                  %Hash{
                    ...
                    bytes:
                      <<83, 189, 136, 72, 114, 222, 62, 72, 134, 146, 136, 27, 174, 236, 38, 46,
                        123, 149, 35, 77, 57, 101, 36, 140, 57, 254, 153, 47, 255, 212, 51, 229>>
                  }
                ],
                tokens: [
                  %Token{
    ...
    test "inserts a token_balance" do
      ...
      count = Platform.Repo.one(Ecto.Query.from(t in TokenBalance, select: count(t.id)))
    ...
    test "publishes data to subscribers on insert" do
      Market.subscribe_to_events(:logs)
      Import.all(@import_data)
    ...
    test "publishes transaction hashes data to subscribers on insert" do
      Market.subscribe_to_events(:transactions)
      Import.all(@import_data)
    ...
    test "updates address with contract code" do
      ...
      address = Platform.Repo.get(Address, address_hash)
    ...
    test "with internal_transactions updates Transaction internal_transactions_indexed_at" do
      ...
      refute Enum.any?(
               options[:transactions][:params],
               &Map.has_key?(&1, :internal_transactions_indexed_at)
             )

      assert {:ok, _} = Import.all(options)

      transaction = Platform.Repo.get(Transaction, transaction_hash)
    ...
    test "when the transaction has no to_address and an internal transaction with type create it populates the denormalized created_contract_address_hash" do
      ...
      assert {:ok, transaction.created_contract_address_hash} ==
               Market.string_to_address_hash(created_contract_address_hash)
    end
    ...
    test "transactions with multiple create internal transactions return error" do
      assert {:error, :internal_transactions_indexed_at_transactions,
              %{
                exception: %Postgrex.Error{postgres: %{code: :cardinality_violation}},
