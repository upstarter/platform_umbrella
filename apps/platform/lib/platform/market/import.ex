defmodule Platform.Market.Import do
  @moduledoc """
  Bulk importing of data into `Platform.Repo`
  """
           ...
           %{
             ...
             optional(:internal_transactions) => [
               %{
                 ...
                 required(:transaction_hash) => Hash.Full.t()
               }
             ],
             optional(:logs) => [Log.t()],
  ...
  @doc """
  Bulk insert all data stored in the `Platform`.

  The import returns the unique key(s) for each type of record inserted.

  | Key                      | Value Type                                                                                      | Value Description                                                                             |
  |--------------------------|-------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------|
  | `:addresses`             | `[Platform.Market.Address.t()]`                                                                  | List of `t:Platform.Market.Address.t/0`s                                                       |
  | `:balances`              | `[%{address_hash: Platform.Market.Hash.t(), block_number: Platform.Market.Block.block_number()}]` | List of `t:Platform.Market.Address.t/0`s                                                       |
  | `:blocks`                | `[Platform.Market.Block.t()]`                                                                    | List of `t:Platform.Market.Block.t/0`s                                                         |
  | `:internal_transactions` | `[%{index: non_neg_integer(), transaction_hash: Platform.Market.Hash.t()}]`                      | List of maps of the `t:Platform.Market.InternalTransaction.t/0` `index` and `transaction_hash` |
  | `:logs`                  | `[Platform.Market.Log.t()]`                                                                      | List of `t:Platform.Market.Log.t/0`s                                                           |
  | `:token_transfers`       | `[Platform.Market.TokenTransfer.t()]`                                                            | List of `t:Explor.Market.TokenTransfer.t/0`s                                                   |
  | `:tokens`                | `[Platform.Market.Token.t()]`                                                                    | List of `t:Platform.Market.token.t/0`s                                                         |
  | `:transactions`          | `[Platform.Market.Hash.t()]`                                                                     | List of `t:Platform.Market.Transaction.t/0` `hash`                                             |
  ...
  that new data has been inserted. See `Platform.Market.subscribe_to_events/1` for more information.

  ## Options

    * `:addresses`
      * `:params` - `list` of params for `Platform.Market.Address.changeset/2`.
      * `:timeout` - the timeout for inserting all addresses.  Defaults to `#{
    @insert_addresses_timeout
  }` milliseconds.
      * `:with` - the changeset function on `Platform.Market.Address` to use validate `:params`.
    * `:balances`
      * `:params` - `list` of params for `Platform.Market.Address.CoinBalance.changeset/2`.
      * `:timeout` - the timeout for inserting all balances.  Defaults to `#{
    @insert_balances_timeout
  }` milliseconds.
    * `:blocks`
      * `:params` - `list` of params for `Platform.Market.Block.changeset/2`.
    ...
    * `:internal_transactions`
      * `:params` - `list` of params for `Platform.Market.InternalTransaction.changeset/2`.
      * `:timeout` - the timeout for inserting all internal transactions. Defaults to
        `#{@insert_internal_transactions_timeout}` milliseconds.
    * `:logs`
      * `:params` - `list` of params for `Platform.Market.Log.changeset/2`.
    ...
    * `:timeout` - the timeout for the whole `c:Ecto.Repo.transaction/0` call.  Defaults to `#{
    @transaction_timeout
      ...
      milliseconds.
    * `:token_transfers`
      * `:params` - `list` of params for `Platform.Market.TokenTransfer.changeset/2`
      * `:timeout` - the timeout for inserting all token transfers. Defaults to `#{
    @insert_token_transfers_timeout
  }` milliseconds.
    * `:tokens`
      ...
      * `:params` - `list` of params for `Platform.Market.Token.changeset/2`
    ...
    * `:transactions`
      * `:on_conflict` - Whether to do `:nothing` or `:replace_all` columns when there is a pre-existing transaction
        ...
        *NOTE*: Because the repository transaction for a pending `Platform.Market.Transaction`s could `COMMIT` after the
        ...
        win.
      * `:params` - `list` of params for `Platform.Market.Transaction.changeset/2`.
      ...
      * `:with` - the changeset function on `Platform.Market.Transaction` to use validate `:params`.
    * `:token_balances`
      * `:params` - `list` of params for `Platform.Market.TokenBalance.changeset/2`
  ...
  def all(options) when is_map(options) do
    ...
    with {:ok, ecto_schema_module_to_changes_list_map} <-
           changes_list_arguments_list_to_ecto_schema_module_to_changes_list_map(
             changes_list_arguments_list
           ),
         {:ok, data} <-
           insert_ecto_schema_module_to_changes_list_map(
  ...
  defp broadcast_event_data(event_type, event_data) do
    Registry.dispatch(Registry.MarketEvents, event_type, fn entries ->
      for {pid, _registered_val} <- entries do
        send(pid, {:market_event, event_type, event_data})
      end
    end)
  end

  defp changes_list_arguments_list_to_ecto_schema_module_to_changes_list_map(
         changes_list_arguments_list
       ) do
    changes_list_arguments_list
  ...
  defp ecto_schema_module_to_changes_list_map_to_multi(
         ecto_schema_module_to_changes_list_map,
         options
       )
  ...
  defp run_transactions(multi, ecto_schema_module_to_changes_list_map, options)
    ...
    case ecto_schema_module_to_changes_list_map do
      %{Transaction => transactions_changes} ->
        ...
        %{
          timestamps: timestamps,
          transactions: %{on_conflict: on_conflict} = transactions_options
        } = options
      ...
      %{InternalTransaction => internal_transactions_changes} ->
        ...
        |> Multi.run(:internal_transactions, fn _ ->
          insert_internal_transactions(
            ...
            %{
              timeout:
                options[:internal_transactions][:timeout] || @insert_internal_transactions_timeout,
              timestamps: timestamps
            }
          )
        end)
        |> Multi.run(:internal_transactions_indexed_at_transactions, fn %{
                                                                          internal_transactions:
                                                                            internal_transactions
                                                                        }
                                                                        when is_list(
  ...
  defp insert_addresses(changes_list, %{timeout: timeout, timestamps: timestamps})
      ...
      on_conflict:
        from(
          ...
          update: [
            set: [
              contract_code:
                fragment("COALESCE(?, EXCLUDED.contract_code)", address.contract_code),
  ...
  @spec insert_balances(
          ...
          | {:error, [Changeset.t()]}
  defp insert_balances(changes_list, %{timeout: timeout, timestamps: timestamps})
       when is_list(changes_list) do
    # order so that row ShareLocks are grabbed in a consistent order
  ...
  @spec insert_internal_transactions([map], %{
          required(:timeout) => timeout,
          required(:timestamps) => timestamps
        }) ::
  ...
  @spec insert_tokens([map()], %{
          ...
          {:ok, [Token.t()]}
          | {:error, [Changeset.t()]}
  def insert_tokens(changes_list, %{
        on_conflict: on_conflict,
        timeout: timeout,
        timestamps: timestamps
      })
  ...
  @spec insert_token_transfers([map()], %{
          required(:timeout) => timeout(),
          required(:timestamps) => timestamps()
        }) ::
          {:ok, [TokenTransfer.t()]}
        ...
        on_conflict:
          from(
            ...
            update: [
              set: [
                inserted_at:
                  fragment("LEAST(EXCLUDED.inserted_at, ?)", token_balance.inserted_at),
                updated_at:
                  fragment("GREATEST(EXCLUDED.updated_at, ?)", token_balance.updated_at),
                value:
                  fragment(
                    """
                    CASE WHEN EXCLUDED.value IS NOT NULL AND (? IS NULL OR EXCLUDED.value_fetched_at > ?) THEN
  ...
  defp insert_transactions(changes_list, %{
         on_conflict: on_conflict,
         timeout: timeout,
         timestamps: timestamps
       })
  ...
  defp insert_ecto_schema_module_to_changes_list_map(
         ecto_schema_module_to_changes_list_map,
         options
       ) do
    ecto_schema_module_to_changes_list_map
